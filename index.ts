 ...
 * @returns 
 */
const composite = async (image1: sharp.Sharp, image2: sharp.Sharp|Array<sharp.OverlayOptions>, x=0, y=0) => {
    if(image2 instanceof Array) {
        return sharp(await sharp(await image1.toBuffer()).composite(image2).toBuffer()).png()
    } else if(image2 instanceof sharp) {
 ...
 * @returns 
 */
const commaSplittedNumber = (num: number, round=-1) => {
...
export const generate = async (character: Character, calcType: ConvAs = "atk"): Promise<Buffer> => {
    ...
    characterStatusPasteList.push(
                { input: await baseHealthImage.toBuffer(), left: 1360 - ((await baseHealthImage.metadata()).width ?? 0) - ((await addHealthImage.metadata()).width ?? 0) - 1, top: 97 + 70*0 },
                { input: await addHealthImage.toBuffer(), left: 1360 - ((await addHealthImage.metadata()).width ?? 0), top: 97 + 70*0 },
                { input: await maxHealthImage.toBuffer(), left: 1360 - ((await maxHealthImage.metadata()).width ?? 0), top: 67 + 70*0 },
                { input: await baseAttackImage.toBuffer(), left: 1360 - ((await baseAttackImage.metadata()).width ?? 0) - ((await addAttackImage.metadata()).width ?? 0) - 1, top: 97 + 70*1 },
                { input: await addAttackImage.toBuffer(), left: 1360 - ((await addAttackImage.metadata()).width ?? 0), top: 97 + 70*1 },
                { input: await attackImage.toBuffer(), left: 1360 - ((await attackImage.metadata()).width ?? 0), top: 67 + 70*1 },
                { input: await baseDefenseImage.toBuffer(), left: 1360 - ((await baseDefenseImage.metadata()).width ?? 0) - ((await addDefenseImage.metadata()).width ?? 0) - 1, top: 97 + 70*2 },
                { input: await addDefenseImage.toBuffer(), left: 1360 - ((await addDefenseImage.metadata()).width ?? 0), top: 97 + 70*2 },
                { input: await defenseImage.toBuffer(), left: 1360 - ((await defenseImage.metadata()).width ?? 0), top: 67 + 70*2 },
                { input: await elementMasteryImage.toBuffer(), left: 1360 - ((await elementMasteryImage.metadata()).width ?? 0), top: 67 + 70*3 },
                { input: await critRateImage.toBuffer(), left: 1360 - ((await critRateImage.metadata()).width ?? 0), top: 67 + 70*4 },
                { input: await critDamageImage.toBuffer(), left: 1360 - ((await critDamageImage.metadata()).width ?? 0), top: 67 + 70*5 },
                { input: await chargeEfficiencyImage.toBuffer(), left: 1360 - ((await chargeEfficiencyImage.metadata()).width ?? 0), top: 67 + 70*6 }
    )
    ...
    if(characterMaxValueStatus.value > 0) {
        ...
        characterStatusPasteList.push(
            ...
            { input: await maxValueStatusImage.toBuffer(), left: 1360 - ((await maxValueStatusImage.metadata()).width ?? 0), top: 67 + 70*7 }
        )
    }
    ...
    for(let i = 0; i < artifacts.length; i++) {
        ...
        let artifactMask = sharp(path.join(assetsPath, "ArtifactMask.png"))
            .resize(332, 332)
        let artifactImage = sharp(path.join(artifactPath, artifact.artifactData.set.name.get("jp"), `${ artifactTypeMap[artifact.artifactData.equipType] }.png`))
            .resize(332, 332)
            .modulate({
                brightness: 0.6,
                saturation: 0.6
        ...
        let artifactLevelRect = roundedRect(45, 24, 0, 0, 2)
        ...
        let subStatusGrowth: {[prop: string]: any} = {}
    ...
    let artifactSet: {[prop: string]: any} = {}
    artifacts.forEach(a => {
        if(a !== null) {
    ...
    for(let i = 0; i < setCount.length; i++) {
        if(artifactSet[setCount[i]] >= 4) {
            ...
            let setCountRect = roundedRect(44, 25, 0, 0, 1)
